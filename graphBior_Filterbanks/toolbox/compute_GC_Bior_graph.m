function [GC GC_inv] = compute_GC_Bior_graph(Ln_bpt,beta,dfactor,hi_d,lo_d,normtype)
...
max_level = 1; % # decomposition levels
...
switch normtype
    case 'asym'
        for level = 1:max_level
            ...
            for i = 1:theta
                fin = ones(N,1);
                fout = sgwt_cheby_op(fin,Ln_bpt{i},c_d{1},arange);
                GC_inv{level,i}(:,1) =fout;
                GC{level,i}(:,1)= 1./(fout+10^-16);
                fin = beta(:,i);
                fout = sgwt_cheby_op(fin,Ln_bpt{i},c_d{2},arange);
