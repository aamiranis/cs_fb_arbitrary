function [beta bptG beta_dist Colorednodes] = harary_decomp(A,F)
% A1 = A;
N = length(A);
Fmin = min(F);
...
beta_dist1 = dec2bin(sort_colors(Fmax,theta));
beta_dist1 = double(beta_dist1) - double('0');
beta_dist1 = 2*beta_dist1 - 1;
beta_dist{1} = beta_dist1;
beta = cell(1,theta);
bptG = cell(1,theta);
for i = 1:theta
    beta{i} = zeros(N,1);
    for j = 1: Fmax
        if beta_dist{1}(j,i) == 1
            beta{1}(Colorednodes{j},i) = 1;
        else
