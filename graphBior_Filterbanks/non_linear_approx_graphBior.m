...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function Output = non_linear_approx_graphBior(varargin)
clc
% close all
disp('This function computes wavelet coefficients of any Image using a 2-dim separable graph-Bior filterbank.')
...
method_name = [];
if edgemap == 1
    method_name = strcat(method_name,'edge-aware ');
end
if strcmp(norm_type,'asym')
    method_name = strcat(method_name,' zeroDC graphBior');
else
    method_name = strcat(method_name,' nonzeroDC graphBior');
end
...
%
% if strcmp(GC_module,'on') && edgemap ==1 && strcmp(norm_type,'asym')
%     figure,wgplot(bptG{1,1}+bptG{1,2},loc{1});
%     axis([95 160 85 160]);
%     set(gca,'Ydir','reverse')
%     figure,wgplot(bptG{2,1}+bptG{2,2},loc{2});
%     axis([95 160 85 160]);
%     set(gca,'Ydir','reverse')
%     figure,wgplot(bptG{3,1}+bptG{3,2},loc{3});
%     axis([95 160 85 160]);
%     set(gca,'Ydir','reverse')
%     figure,wgplot(bptG{4,1}+bptG{4,2},loc{4});
%     axis([95 160 85 160]);
%     set(gca,'Ydir','reverse')
% end
...
%         switch norm_type
%             case 'sym'
%                 GC = compute_GC_Bior1(Ln_bpt,[], hi_d,lo_d, norm_type);
...
%                     end
%                 end
%                 GC = compute_GC_Bior1(Ln_sym,dfactor, hi_d,lo_d, norm_type);
%         end
%     case 'off'
%         %         switch norm_type
%         %             case 'sym'
...
for iter = 1:length(nnz)
    ...
    if iter == 4
        ...
        high_in = max(f_hat1);
        high_out = 255;
        distImg1 = high_out*(distImg - low_in)/(high_in - low_in);
        imagesc(distImg1);
%         axis([95 160 85 160]);
        colorbar
        colormap(gray)
        set(gca,'xTick',[])
        set(gca,'yTick',[])
        title(sprintf('%s%1.2f%s%s ', 'Reconstruction with ', nnz_factor, ' highpass coefficients using ',method_name));
    end
